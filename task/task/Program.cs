...
namespace task
{
    class Program
    {
        ...
        static void Main()
        {
            ...
            //GetEmailRtyhaAndDayrep(debtors);
            //GetAge26and36(debtors);
            //GetNotMoreThan5000(debtors);
            //GetNameMoreThan18AndPhone(debtors);
            //GetWinterBorn(debtors);
        ...
        {
            var result = debtors.Where(debtor => debtor.Email.Substring(debtor.Email.IndexOf('@') + 1) == "rhyta.com" || debtor.Email.Substring(debtor.Email.IndexOf('@') + 1) == "dayrep.com").ToList();
            Output(result);
        }

        static void GetAge26and36(List<Debtor> debtors)
        {
            DateTime time = DateTime.Now;
            var result = debtors.Where(debtor => (time - debtor.BirthDay).TotalMinutes / 525600 >= 26 && (time - debtor.BirthDay).TotalMinutes / 525600 <= 36).ToList();
            Output(result);
        }

        static void GetNotMoreThan5000(List<Debtor> debtors)
        {
            var result = debtors.Where(debtor => debtor.Debt < 5000).ToList();
            Output(result);
        }

        static void GetNameMoreThan18AndPhone(List<Debtor> debtors)
        {
            var result = debtors.Where(debtor => debtor.FullName.Length > 18 && (debtor.Phone.ToCharArray().Count(f => f == '7') >= 2)).ToList();
            Output(result);
        }

        static void GetWinterBorn(List<Debtor> debtors)
        {
            var result = debtors.Where(debtor => (debtor.BirthDay.Month == 1 || debtor.BirthDay.Month == 2 || debtor.BirthDay.Month == 12)).ToList();
            Output(result);
        }

        static void GetDebtMoreThanAverage(List<Debtor> debtors)
        {
            double average = 0;
            ...
            average = average / debtors.Count();
            var result = debtors.Where(debtor => debtor.Debt > average).ToList();
            result = result.OrderBy(x => x.FullName).OrderBy(x => x.Debt).Reverse().ToList();
            Output(result);
        }

        static void GetPhoneDoesntExist8(List<Debtor> debtors)
        {
            var result = debtors.Where(debtor => debtor.Phone.ToCharArray().Count(f => f == '8') == 0).ToList();
            Output2(result);
        }

        static void GetNameWith3SameChar(List<Debtor> debtors)
        {
            List<Debtor> result = new List<Debtor>();
        ...
        static void Output2(List<Debtor> list)
        {
            ...
            {
                Console.WriteLine(item.FullName + "  " + (time - item.BirthDay).TotalMinutes / 525600 + "  " + item.Debt);
            }
        }
        static void Output(List<Debtor> list)
        {
            foreach (var item in list)
            {
